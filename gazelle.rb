...
module RubyGazelle
  class Gazelle
    ...
    REQUESTS = {index: '/ajax.php?action=index', user_profile: '/ajax.php?action=user', inbox: '/ajax.php?action=inbox',
      ...
      torrent: '/ajax.php?action=torrent'}
    ...
    def initialize(login_page, username, password)
      ...
      if cf_matches = /__cfduid=[^;]*/.match(set_cookie)
        @cookie = "#{cf_matches[0]}; #{@cookie}"
      end
    end
    ...
    def torrent(id)
      make_request(REQUESTS[:torrent], :id => id)
    end
    ...
    def make_request(page, options = {})
      ...
      unless status.eql? 'success'
        raise Exception, "Failed request to page=#{page} status=#{status}. Response body: #{response}"
      end

      if parsed_response.is_a?(Array)
        parsed_response.collect{|el| RecursiveOpenStruct.new(el)}
      else
        RecursiveOpenStruct.new(parsed_response)
      end
    end
  end
end
